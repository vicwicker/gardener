...
var _ = Describe("PrometheusRules", func() {
	Describe("#CentralPrometheusRules", func() {
		...
		It("should return the expected objects when seed is also the garden cluster", func() {
			seedIsGarden := true
		...
		It("should return the expected objects when seed and the garden clusters are different", func() {
			seedIsGarden := false
...
>
    ( {__name__=~"kube_customresource_verticalpodautoscaler_status_recommendation_containerrecommendations_target_.+"} > 0 )`),
				...
				Annotations: map[string]string{
					...
					"description": "The {{ $labels.verticalpodautoscaler }} VPA " +
						"in the {{ $labels.namespace }} namespace " +
						"in the {{ $externalLabels.seed }} seed has a " +
						"{{ if eq .Labels.unit \"core\" -}} CPU {{- else if eq .Labels.unit \"byte\" -}} memory {{- end }}" +
						"uncapped target recommendation larger than the regular target recommendation for the {{ $labels.container }} container.",
				},
			}
			Expect(aggregate.CentralPrometheusRules(seedIsGarden)).To(HaveExactElements(
				PointTo(MatchFields(IgnoreExtras, Fields{
