...
type GardenerDashboardConfig struct {
	...
	// SNI TLS secret to the dashboard's apiServerCaData configuration (used for all kubeconfigs
	// generated by the dashboard, e.g. for terminal sessions, OIDC login, or service account access).
	// Enable this when the CA that signed the SNI TLS certificate is not present
	// in the clients' trust stores (e.g. a private CA not distributed to end users).
	// When the CA is already trusted by clients — whether publicly trusted (e.g. Let's Encrypt)
	// or a corporate CA installed in client trust stores — leave this disabled to allow
	// transparent CA rotation without requiring kubeconfig re-downloads.
	// This field is only relevant when .spec.virtualCluster.kubernetes.kubeAPIServer.sni is configured.
	// Defaults to false.
	// +optional
	PropagateCAFromSNI *bool `json:"propagateCAFromSNI,omitempty"`
}
